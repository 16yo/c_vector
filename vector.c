...
vector create_empty_vector() {
    return create_vector(0, NULL);
}

vector create_vector_by_size(const uint64_t size, const int64_t fill) {
    int64_t* p = (int64_t*)malloc(size * sizeof(int64_t));
    for (uint64_t i = 0; i < size; ++i) {
        p[i] = fill;
    }
    create_vector(size, p);
}
...
void resize_vector(vector* v, uint64_t size, int64_t fill) {
    if (v->size == size) {
        return;
    }
